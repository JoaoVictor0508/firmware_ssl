...
#define _NRF24L01P_CHANNEL_A_RX 75
#define _NRF24L01P_CHANNEL_A_TX 117

#define _NRF24L01P_CHANNEL_B_RX 87
#define _NRF24L01P_CHANNEL_B_TX 91

#define MAX_SPI_BUF_SIZE 32 // Tamanho do Buffer da SPI
...
#define PACKET_END 'e'
#define ROBOFEI_IDENTIFIER 0x90
#define SEND_FEEDBACK 0x80

#define KICK_TYPE_MASK 0x80
#define KICK_STRENGTH_MASK 0x7F
...
#define nRF24_CMD_ACTIVATE                                                     \
    (uint8_t)0x50 // (De)Activates R_RX_PL_WID, W_ACK_PAYLOAD,
                  // W_TX_PAYLOAD_NOACK features
#define nRF24_CMD_R_RX_PL_WID                                                  \
    (uint8_t)0x60 // Read RX-payload width for the top R_RX_PAYLOAD in the RX
                  // FIFO.
...
#define nRF24_CMD_W_TX_PAYLOAD_NOACK                                           \
    (uint8_t)0xB0                        // Write TX payload and disable AUTOACK
...
#define nRF24_CMD_NOP                                                          \
    (uint8_t)0xFF // No operation (used for reading status register)
...
#define nRF24_REG_RX_PW_P0                                                     \
    (uint8_t)0x11 // Number of bytes in RX payload in data pipe 0
#define nRF24_REG_RX_PW_P1                                                     \
    (uint8_t)0x12 // Number of bytes in RX payload in data pipe 1
#define nRF24_REG_RX_PW_P2                                                     \
    (uint8_t)0x13 // Number of bytes in RX payload in data pipe 2
#define nRF24_REG_RX_PW_P3                                                     \
    (uint8_t)0x14 // Number of bytes in RX payload in data pipe 3
#define nRF24_REG_RX_PW_P4                                                     \
    (uint8_t)0x15 // Number of bytes in RX payload in data pipe 4
#define nRF24_REG_RX_PW_P5                                                     \
    (uint8_t)0x16 // Number of bytes in RX payload in data pipe 5
...
#define nRF24_FEATURE_EN_DYN_ACK                                               \
    (uint8_t)0x01 // EN_DYN_ACK bit in FEATURE register
#define nRF24_FEATURE_EN_ACK_PAY                                               \
    (uint8_t)0x02                          // EN_ACK_PAY bit in FEATURE register
#define nRF24_FEATURE_EN_DPL (uint8_t)0x04 // EN_DPL bit in FEATURE register
#define nRF24_FLAG_RX_DR                                                       \
    (uint8_t)0x40 // RX_DR bit (data ready RX FIFO interrupt)
#define nRF24_FLAG_TX_DS                                                       \
    (uint8_t)0x20 // TX_DS bit (data sent TX FIFO interrupt)
#define nRF24_FLAG_MAX_RT                                                      \
    (uint8_t)0x10 // MAX_RT bit (maximum number of TX retransmits interrupt)

// Register masks definitions
#define nRF24_MASK_REG_MAP                                                     \
    (uint8_t)0x1F // Mask bits[4:0] for CMD_RREG and CMD_WREG commands
#define nRF24_MASK_CRC                                                         \
    (uint8_t)0x0C // Mask for CRC bits [3:2] in CONFIG register
#define nRF24_MASK_STATUS_IRQ                                                  \
    (uint8_t)0x70 // Mask for all IRQ bits in STATUS register
#define nRF24_MASK_RF_PWR                                                      \
    (uint8_t)0x06 // Mask RF_PWR[2:1] bits in RF_SETUP register
#define nRF24_MASK_RX_P_NO                                                     \
    (uint8_t)0x0E // Mask RX_P_NO[3:1] bits in STATUS register
#define nRF24_MASK_DATARATE                                                    \
    (uint8_t)0x28 // Mask RD_DR_[5,3] bits in RF_SETUP register
#define nRF24_MASK_EN_RX                                                       \
    (uint8_t)0x3F // Mask ERX_P[5:0] bits in EN_RXADDR register
#define nRF24_MASK_RX_PW (uint8_t)0x3F // Mask [5:0] bits in RX_PW_Px register
#define nRF24_MASK_RETR_ARD                                                    \
    (uint8_t)0xF0 // Mask for ARD[7:4] bits in SETUP_RETR register
#define nRF24_MASK_RETR_ARC                                                    \
    (uint8_t)0x0F // Mask for ARC[3:0] bits in SETUP_RETR register
#define nRF24_MASK_RXFIFO                                                      \
    (uint8_t)0x03 // Mask for RX FIFO status bits [1:0] in FIFO_STATUS register
#define nRF24_MASK_TXFIFO                                                      \
    (uint8_t)0x30 // Mask for TX FIFO status bits [5:4] in FIFO_STATUS register
#define nRF24_MASK_PLOS_CNT                                                    \
    (uint8_t)0xF0 // Mask for PLOS_CNT[7:4] bits in OBSERVE_TX register
#define nRF24_MASK_ARC_CNT                                                     \
    (uint8_t)0x0F // Mask for ARC_CNT[3:0] bits in OBSERVE_TX register
...
{
    nRF24_ARD_NONE =
        (uint8_t)0x00, // Dummy value for case when retransmission is not used
...
enum
{
    nRF24_STATUS_RXFIFO_DATA =
        (uint8_t)0x00, // The RX FIFO contains data and available locations
    ...
    nRF24_STATUS_RXFIFO_ERROR =
        (uint8_t)0x03 // Impossible state: RX FIFO cannot be empty and full at
                      // the same time
};
...
enum
{
    nRF24_STATUS_TXFIFO_DATA =
        (uint8_t)0x00, // The TX FIFO contains data and available locations
    ...
    nRF24_STATUS_TXFIFO_ERROR =
        (uint8_t)0x03 // Impossible state: TX FIFO cannot be empty and full at
                      // the same time
};
...
static const uint8_t nRF24_RX_PW_PIPE[6] = {
    nRF24_REG_RX_PW_P0, nRF24_REG_RX_PW_P1, nRF24_REG_RX_PW_P2,
...
static const uint8_t nRF24_ADDR_REGS[7] = {
    nRF24_REG_RX_ADDR_P0, nRF24_REG_RX_ADDR_P1, nRF24_REG_RX_ADDR_P2,
...
typedef struct
{
    uint8_t rxBuffer[MAX_SPI_BUF_SIZE];
    uint8_t txBuffer[MAX_SPI_BUF_SIZE];
...
typedef enum
{
    ID_P = PREAMBLE_SZ + 1,   // Posiçãoo do ID do robô no pacote
    VX_P = PREAMBLE_SZ + 2,   // Posiçãoo da velocidade em X no pacote
    VY_P = PREAMBLE_SZ + 3,   // Posiçãoo da velocidade em Y no pacote
    VW_P = PREAMBLE_SZ + 4,   // Posiçãoo da velocidade de rotação no pacote
    // KICK_P = PREAMBLE_SZ + 6, // Posiçãoo do comando de chute no pacote
    // ROLLER_P = PREAMBLE_SZ + 6, // Posiçãoo do comando do roller no pacote
    FEEDBACK_P = PREAMBLE_SZ + 8, // Posiçãoo do comando de enviar feedback no pacote
    KICK_CFG_P = PREAMBLE_SZ + 9, // Posiçãoo da força do chute no pacote
    ROLLER_V_P = PREAMBLE_SZ + 10, // Posiçãoo da velocidade do roller no pacote
    X_LSB_P = PREAMBLE_SZ + 11, // Posição da parte baixa da posição X
...
static inline uint8_t nRF24_LL_RW(uint8_t data)
{
    ...
    uint8_t result;
    HAL_StatusTypeDef spiResult =
        HAL_SPI_TransmitReceive(radioSPI, &data, &result, 1, 5);
    while (spiResult == HAL_BUSY)
    {
        spiResult = HAL_SPI_TransmitReceive(radioSPI, &data, &result, 1, 5);
    }
    if (spiResult != HAL_OK)
    {
        Error_Handler();
